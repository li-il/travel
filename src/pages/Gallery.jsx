    ...
    fetch("./world.json")
      ...
      .then(worldData => {
        ...
        travelSpots.forEach(spot => {
          ...
          if (country) {
            visitedSet.add(country.id || country.properties.name);
          }
        });
      ...
      <div className="max-w-7xl mx-auto relative">
        ...
        <header className="relative z-10 mb-24 flex flex-col md:flex-row md:items-end justify-between gap-8">
          ...
          <motion.div 
            ...
            className="glass-panel grid grid-cols-2 md:flex gap-4 md:gap-12 p-8 rounded-[2rem] flex-grow ml-0 md:ml-12"
          >
            <div className="flex gap-8 md:gap-12 border-r border-white/10 pr-8 md:pr-12">
            ...
            {/* 全球探索进度条 */}
            <div className="flex-grow w-full min-w-[150px]">
              ...
              <div className="columns-1 md:columns-2 lg:columns-3 gap-8 space-y-8">
                {groupedPhotos[date].map((photo, photoIndex) => (
                  <div key={photoIndex} className="group relative cursor-pointer overflow-hidden rounded-[2.5rem] bg-slate-800 border border-white/10 transition-all duration-500 hover:border-blue-400/50 hover:shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
                    <img src={photo.url} className="w-full h-auto object-cover transition-all duration-1000 group-hover:scale-105 group-hover:brightness-75" />
